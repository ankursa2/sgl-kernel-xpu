...
def ref_mla_prefill_lse_varlen(
    q_nope: Tensor,  # (total_q, H, D_latent)
    q_pe: Tensor,  # (total_q, H, D_rope)
    kv_cache: Tensor,  # (total_pages, page_size, D_ckv)
    scale: float,
    ...
    """Reference causal-masked logsumexp (log base 2) per query row, matching
    flash_mla_prefill's documented lse convention."""
    batch_size = seq_lens_k.shape[0]
    H = q_nope.shape[1]
    D_ckv = q_nope.shape[2] + q_pe.shape[2]
    total_q = q_nope.shape[0]

    lse = torch.zeros(total_q, H, dtype=torch.float32)
    ...
    for b in range(batch_size):
        ...
        q_full = torch.cat([q_nope[q_start:q_end], q_pe[q_start:q_end]], dim=-1).float()
        kv = kv_cache[block_tables[b]].reshape(-1, D_ckv)[:seqlen_k].float()

        causal_offset = seqlen_k - seqlen_q
        q_idx = torch.arange(seqlen_q).unsqueeze(1)
        k_idx = torch.arange(seqlen_k).unsqueeze(0)
        mask = k_idx > (causal_offset + q_idx)  # (seqlen_q, seqlen_k)

        for h in range(H):
            scores = (q_full[:, h] @ kv.T) * scale  # (seqlen_q, seqlen_k)
            scores = scores.masked_fill(mask, float("-inf"))
            lse[q_start:q_end, h] = torch.logsumexp(scores, dim=-1) / math.log(2)
...
@pytest.mark.parametrize(
    ...
    ids=["bs1", "bs2_uniform", "bs3_mixed"],
)
def test_mla_prefill_return_lse(dtype, num_heads, seqlens_q, seqlens_k):
    """return_lse=True: lse must match causal-masked logsumexp(QK^T * scale)
    in log base 2, per query row."""
    ...
    lse_ref = ref_mla_prefill_lse_varlen(
        q_nope_cpu, q_pe_cpu, kv_cache_cpu, scale, block_table_cpu, cu_seqlens_q, seq_lens_k
    )
    ...
    assert out.shape == (total_q, num_heads, D_latent)
